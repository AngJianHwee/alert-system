...
const Alert: React.FC<AlertProps> = ({ alert, onDismiss }) => (
  ...
  >
    <div className="p-4">
      ...
      <p className="text-xs mb-3 text-gray-800 animate-in fade-in duration-200 delay-100" title={alert.message}>
        {alert.message.length > 100 ? `${alert.message.substring(0, 97)}...` : alert.message}
      </p>
      <div className="text-[10px] italic text-gray-700 mb-3 animate-in fade-in duration-200 delay-200">
        {alert.timestamp.toLocaleString()} • {formatRelativeTime(alert.timestamp)}
      </div>
      <div className="flex gap-2">
        <Button
          variant="ghost"
          className="!rounded-button text-xs h-8 whitespace-nowrap cursor-pointer hover:opacity-80 hover:scale-105 transition-all duration-200 text-white"
          style={{ backgroundColor: colors[alert.type].border }}
          onClick={(e) => {
            const alertElement = e.currentTarget.closest('.rounded-lg.shadow-lg'); // Get the root alert div
            if (alertElement instanceof HTMLElement) {
              addAnimation(alertElement); // Apply animation directly to the alert
